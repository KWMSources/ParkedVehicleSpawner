...
export default class ParkedVehicleSpawner {
    /**
     ...
     * @memberof ParkedVehicleSpawner
     */
    static spawn(amount = 1000) {
        let vehicleGens = vehicleGenerators.filter((generator) => {
            if (
                generator.Flags > 5000 ||
                !ALLOWED_FLAGS.includes(generator.Flags)
            )
                return false;
            return true;
        });
        ...
        for (
            ...
            this._takenSpots.push(spawnTest);
            ...
            if (item.CarModel !== "") model = item.CarModel;
                ...
                model =
                    vehiclePopGroups[item.PopGroup][
                        getRandomInt(
                            0,
                            vehiclePopGroups[item.PopGroup].length - 1
                        )
                    ];
            else
                model =
            ...
            if (!colorlessVehicles.includes(model)) {
                if (vehicleColors.length === 0)
                    ...
                    let color =
                        vehicleColors[
                            getRandomInt(0, vehicleColors.length - 1)
                        ];
                    vehicle.customPrimaryColor = new alt.RGBA(
                        color.r,
                        color.g,
        ...
        alt.log(`Created ${this._vehicles.length} generated vehicles`);
    }
    /**
     ...
     */
    static clear() {
        let vehs = Array.from(this._vehicles);
        this._vehicles = [];
